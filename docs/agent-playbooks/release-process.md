...
./arti-bin/<platform>-<arch>/arti --version      # arti.exe on Windows
```
...
**License check.** `NOTICES`, `LICENSE_AUDIT.md`, and `licenses-audit.json` must describe what the artifacts actually contain. Most of that is now enforced: `licenses-audit.test.js` derives the bundled-component inventory from `package.json`'s `build.extraResources` (every scope) and from everything `build.files`' `src/**/*` pattern commits into `app.asar` — `src/renderer/vendor/` and the assets beside it — fails on any component that ships without being classified and attributed, holds the audit's baseline to `package.json`'s version, cross-checks each recorded version against its pin (`fetch-ant.js`, `fetch-myotis.js`, `fetch-arti.js`, `fetch-freedom-ipfs-native.js`, `radicle-addon-version.js`, the lockfile), and rejects any GPL/AGPL-licensed vendor file or production package. It runs in the normal `npm test` matrix, so a red branch tells you before you tag.

Presence in the _artifact_ is checked separately, because config alone cannot prove it: `test-e2e/packaged/notices.spec.js` runs on all six release smoke legs (§6) and asserts the built package really carries `resources/NOTICES`, `resources/LICENSE` and Chromium's `LICENSES.chromium.html`. macOS is why — electron-builder deletes the `LICENSES.chromium.html` Electron ships beside the executable when it builds a `.app`, and the `.dmg`/`-mac.zip` carry only the bundle, so `build.mac.extraResources` puts a copy inside `Contents/Resources` and this spec is what notices if that entry ever goes.
...
**CI is green** on the release branch head you are about to tag (`gh pr checks` on the branch's PR, or the Actions tab). The release workflow does not gate on CI, so a red branch produces a red release. Green on **attempt 2** counts as green: `.github/workflows/auto-retry.yml` re-runs a failed run's failed jobs once, automatically (Appendix B). Green on attempt 1 after a red attempt is not a thing — the retry only ever moves a run from red towards green, never the reverse — but a run that is *still* red on attempt 2 has failed twice and is a real failure to investigate, not a flake to re-run a third time.
...
- macOS actually handing the camera and microphone to the signed app after an in-app Allow: that is the TCC privacy dialog, and a hosted Mac runner has no camera, cannot drive the dialog headlessly, and cannot pre-seed the TCC database (SIP blocks it). Chromium's `--use-fake-device-for-media-stream` would bypass the exact OS path [#362](https://github.com/solardev-xyz/freedom-browser/issues/362) fixed, so it proves nothing. CI gets as far as asserting the entitlements and usage descriptions are in the signed bundle (step 7 below is the rest)
...
5. **Bundled nodes**: confirm Ant, native IPFS, and Radicle start cleanly (Radicle ships on macOS, Linux, and Windows), and that Tor reaches "running" from the node-status menu on every artifact this release publishes (macOS arm64, Linux x64/arm64 and Windows x64 all bundle Arti; releases cut before Windows Arti bundling landed in September 2026 have none on Windows). The nodes manager or the relevant `freedom://` settings page surfaces this — a "node failed to start" red badge or a missing native addon/API port is the failure mode
6. **Persistence**: change one trivial setting (e.g. theme), close the app fully, reopen, confirm the change stuck
7. **Camera and microphone (macOS only)**: on a Mac, start a Google Meet call in the packaged app; Allow must reach the OS prompt and the camera must work

**Every platform runs steps 1, 2, 3, 5 and 6 automatically.** The four `smoke-*` jobs in `.github/workflows/release.yml` drive the run's own artifacts with two Playwright projects (`npm run test:e2e:packaged` runs both), each on a runner of the target OS and arch, and each twice because every platform ships two things a user can install: macOS the app copied out of the mounted `.dmg` and the app from the `-mac.zip`; Linux (x64 and arm64) `/opt/Freedom/freedom` from the installed `.deb` and the binary inside the extracted AppImage; Windows `%LOCALAPPDATA%\Programs\freedom-browser\Freedom.exe` from the silently installed NSIS package and `Freedom.exe` from the portable zip. `packaged` covers steps 1, 2 and 6 (launch, version, persistence) with the app's nodes and network stubbed out; `packaged-live` launches the same artifact without the test harness and covers steps 3 and 5 — a local page and `https://example.com` render with the shield in its default state, and the bundled Ant (including a `/health` call on the port the app publishes), native IPFS and Radicle each reach "running". Tor is asserted the same way wherever the build bundles Arti — the shipped `arti` binary must run (`arti --version`) and the node must reach "running" — and is reported as a skip with its reason where the build bundles none (any build made without `npm run tor:download`; every artifact this workflow publishes bundles one) or where Arti cannot bootstrap a circuit inside `tor-manager`'s own ~120s budget, which is a property of the runner's network path rather than of the artifact. Because that leg skips itself rather than failing when no Arti is present, every smoke job additionally asserts the file itself in both of the artifacts its platform ships — `resources/arti-bin/arti` inside the `.deb` and the extracted AppImage, `Freedom.app/Contents/Resources/arti-bin/arti` in the app copied out of the `.dmg` and in the one from the `-mac.zip`, `resources\arti-bin\arti.exe` in the installed package and the portable zip — so a dropped `extraResources` entry cannot pass as a skip on any platform. Those assertions are gated on the same `BUNDLE_TOR` expression as the build jobs, so a `bundle_tor=false` dispatch run still passes. `release` depends on all four jobs, so an artifact that cannot launch, reports the wrong version, cannot render a page, cannot start its nodes, or loses a setting across a restart never reaches a release page. Each job passes the tag version as `FREEDOM_E2E_EXPECTED_VERSION`, which is exactly the step-2 check; a signed mac run additionally asserts `spctl` accepts the app copied out of the disk image. Two checks that are not checklist steps ride along on every leg for the same reason — they are packaging properties nothing else can see. `test-e2e/packaged/permissions.spec.js` re-checks the site-permission fixes in the built app on all six legs: an undecided site reads as promptable rather than blocked ([#361](https://github.com/solardev-xyz/freedom-browser/issues/361)), three dismissals of the anchored prompt embargo it and the fourth request is auto-denied with no prompt ([#364](https://github.com/solardev-xyz/freedom-browser/issues/364)), and an Allow with "Remember for this site" survives a full quit and relaunch against the packaged profile. And on a **signed** mac run only, both mac legs read the entitlements back out of the bundle's own signature — the outer `Freedom.app` and every helper bundle under `Contents/Frameworks`, which host the capture processes and are signed separately through `entitlementsInherit` (`node scripts/check-mac-entitlements.js`, mutation-tested off a Mac by `scripts/check-mac-entitlements.test.js`) — and fail the job unless every key `config/entitlements.mac.plist` grants is there in each of them — `com.apple.security.device.camera` and `.audio-input` in particular — plus `NSCameraUsageDescription` / `NSMicrophoneUsageDescription` in `Info.plist` ([#362](https://github.com/solardev-xyz/freedom-browser/issues/362)). It is gated on `SIGNED` because entitlements only reach a bundle through the `codesign` pass an unsigned dispatch run skips, so there is nothing to assert there; what `codesign -d` does on such a build (error out, or print an empty set off the ad-hoc signature the repacked Electron binaries shipped with) has not been checked against a real unsigned artifact, and the gate holds either way. The source config itself is guarded on every pull request by `config/entitlements.mac.test.js`. Step 4 (headline feature), step 7 (the macOS camera/microphone pass, which needs the TCC dialog), the wallet, and the upgrade-from-previous-version pass are **not** automated on any platform and are still done by hand as described above. To run the automated legs yourself against any packaged build:
...
`.github/workflows/release.yml` also runs on a schedule, `7 22 * * *` — **22:07 UTC, 00:07 CEST / 23:07 CET**. GitHub has fired this repo's schedule about five hours late on every observed night regardless of the minute chosen, and a signed nightly takes about 35 minutes, so this slot lands the build before 07:00 CEST even with a six-hour delay and simply earlier on a punctual night. The schedule only fires on `main` (GitHub runs scheduled workflows from the default branch).
...
| smoke jobs       | The same §6 steps 1/2/6 and site-permission checks against every artifact, asserting the nightly version            |
...
**Internal testing only.** A nightly is whatever was on `main` when the schedule fired (22:07 UTC, in practice often hours later). It has passed the packaged smoke tests and nothing else — no §6 pass, no manual checklist, no changelog. Do not hand it to users.
...
`electron-builder` cross-builds the Windows NSIS installer and zip from the mac host — no Windows machine required. Windows x64 and ARM64 builds include the architecture-matched embedded Radicle node. They do **not** include Tor: Arti is compiled for the host, so `arti-bin/win-x64/arti.exe` only exists when `npm run tor:download` ran on a Windows machine. The release workflow's `windows-x64` job does exactly that, which is why its artifacts bundle Tor and a cross-built local one does not.
...
- **Re-runs**: "Re-run failed jobs" keeps the successful legs' artifacts and re-runs `release`. "Re-run all jobs" rebuilds everything, including a fresh macOS signature and notarization — the bytes and hashes change, which is fine for a draft and forbidden for a published release (the guard above). The first "Re-run failed jobs" now happens by itself — see **Automatic retries** below.
- **Automatic retries, once per run.** `.github/workflows/auto-retry.yml` watches CI and Release (`on: workflow_run`) and presses "Re-run failed jobs" for you when a run ends `failure`, `cancelled` or `timed_out` on its **first** attempt; the rules live in `scripts/ci/auto-retry.sh`. Reading a run in this world:
  - **Telling one apart**: the run page shows an attempt selector ("Attempt #2") at the top right, `gh run view <id> --json attempt` reports `2`, and the Auto-retry workflow's own run logs one line naming the run URL and the jobs it re-ran. A run with no attempt selector was never retried.
  - **A second failure is genuine.** Attempt 2 is never retried, so anything red after a retry has failed twice and needs a look, not another button press.
  - **`cancelled` is retried on purpose**: a job killed by `timeout-minutes` reports the *run* as cancelled (observed as of 2026-09-17, run 35130040315), and a hang is exactly what this absorbs (`main` went red from one on 2026-09-16). `timed_out` — the conclusion the REST API documents for that state — is accepted at both the run and the job level too, so the retry does not depend on which of the two GitHub reports. `main` is retried like any other branch. A run superseded by a newer push on the same branch is *not* retried — `ci.yml` cancels those itself, and re-testing an obsolete commit helps nobody. A run a human cancelled deliberately looks identical to a timed-out one in the event payload and will be re-run once; cancelling attempt 2 sticks.
  - It only ever re-runs jobs. It never comments on a pull request, opens an issue, or touches the release.
...
- **Known first-run fixes** worth remembering when a leg breaks: every fetch script downloads through `scripts/lib/fetch-with-retry.js`, which uses a retrying `https` client rather than the global `fetch` because the EasyList server drops connections mid-body and undici turns that into an *uncaught* assertion (see §2, "Bundled binaries"); the Ant and IPFS fetch scripts extract archives with Windows' own bsdtar and relative paths because the Windows job runs in Git Bash, where GNU tar misreads `D:\...` as a remote host.
- **Network steps are bounded and retried, never raw.** Three classes of network step in CI can trickle forever instead of failing, and the only thing that ends either is the job's `timeout-minutes` — which reports as a _cancelled_ job, burns the whole budget, and says nothing about what stalled. Each has a wrapper, and any new step that installs or downloads something goes through the matching one rather than calling the tool directly (the third, `scripts/lib/fetch-with-retry.js` for every `scripts/fetch-*.js` binary download, is described in §2 under "Bundled binaries"):
  - **apt / `playwright install-deps`** (Linux legs): `scripts/ci/apt-hardening.sh`, used through the `.github/actions/install-playwright-deps` and `.github/actions/install-linux-native-deps` composite actions. Per-attempt `timeout`, retries, a dpkg/apt lock wait, and an azure → `archive.ubuntu.com` mirror fallback on the final attempt, because a degraded Azure mirror backend trickles a fetch at ~35 kB/s rather than failing and apt's own `Acquire::*::Timeout` is an inactivity timeout that never fires while bytes keep arriving ([#203](https://github.com/solardev-xyz/freedom-browser/pull/203)).
  - **`npm ci`** (every leg, every OS): the `.github/actions/install-node-deps` composite action, which is what every `Install dependencies` step in `ci.yml` and `release.yml` now `uses:` — pass `ignore-scripts: 'true'` for the jobs that only need the JavaScript on disk. It sets npm's own fetch bounds for the job (`fetch-retries 5`, `fetch-timeout 120000`), restores the Electron download cache, and then runs `npm ci` through `scripts/ci/npm-ci-hardening.js`: **3 attempts, 540s per attempt and 660s for the whole step on Windows, 300s/600s everywhere else**, killing the process _tree_ on a timeout (the stall can sit inside a grandchild — a registry fetch, or a `postinstall` lifecycle script — which survives signalling `npm` alone) and clearing `node_modules` between attempts. Both numbers are sized from measured step durations, and Windows has its own pair because its distribution genuinely differs — a healthy install has been clocked at up to 337s there against 77s everywhere else, on a run where three sibling Windows jobs finished the same install in 106-122s. The step budget is the load-bearing half: three full attempts always exceed it, and most `ci.yml` jobs cap themselves at `timeout-minutes: 15`, so without a total bound a stall would still end as a cancellation. Raising it means re-checking `budget + slowest preflight < 15 min` (the slowest checkout + setup-node observed is 102s); `scripts/ci/npm-ci-hardening.test.js` pins that arithmetic. Override per job with `FREEDOM_CI_NPM_ATTEMPTS` / `FREEDOM_CI_NPM_ATTEMPT_TIMEOUT` / `FREEDOM_CI_NPM_TOTAL_TIMEOUT` (seconds). The Electron cache is keyed on the exact `electron` version in `package-lock.json` plus the runner's OS and arch, and each job logs `Electron download cache HIT`/`MISS` before installing; it is skipped for `ignore-scripts: 'true'` jobs, which never launch Electron and so have nothing to cache. `npm ci` itself never downloads Electron: the `electron` package ships no postinstall script, and `node_modules/electron/index.js` fetches the ~100 MB zip on the first `require('electron')` — a Playwright launch or a build, i.e. a later step of the job — which is also why a job that sets `ignore-scripts: 'true'` and then does require Electron would download it uncached. `postinstall`'s other half, `scripts/better-sqlite3-prebuilds.js`, downloads nothing — the prebuilt addons ship inside the npm tarball `actions/setup-node`'s `cache: npm` already covers — so there is no second directory to cache ([#379](https://github.com/solardev-xyz/freedom-browser/issues/379)).
- **Smoke gate**: `smoke-linux` (a matrix over x64/arm64), `smoke-mac-arm64` and `smoke-windows-x64` each run after their build job and before `release` (§6). They need no secrets and rebuild nothing — a scripts-free install (`install-node-deps` with `ignore-scripts: 'true'`), then Playwright against the run's own artifacts. A failure in any of them blocks the release job; each uploads its Playwright traces as `smoke-<platform>-report` (deliberately not matching the `freedom-*` pattern the `release` job downloads, so traces can never become release assets). The mac job reads the signing state off the artifact name it downloaded (`freedom-mac-arm64-signed` / `-unsigned`) and only assesses Gatekeeper when there is a signature to assess, so an unsigned dispatch run still passes.
