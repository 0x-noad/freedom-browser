...
- **Dependency installs are bounded and retried, never raw.** Two classes of install step in CI can trickle forever instead of failing, and the only thing that ends either is the job's `timeout-minutes` — which reports as a _cancelled_ job, burns the whole budget, and says nothing about what stalled. Both have a wrapper, and any new step that installs something goes through the matching one rather than calling the tool directly:
